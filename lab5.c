 ...
 *
 * Assumptions: The program will accept command line input from the user
 * 	to control which driver is used, and which settings are used for
...
int main(int argc, char **argv)
{
	...
	const int del[] = {10};
	 ...
	 // this driver tests the functionality of the bst_int_path_len function,
	 // which finds the internal path length of the tree.
	 if (UnitNumber == 7) {
		 const int ins[] = {5, 3, 7, 9, 2, 10};
		 int *testData0 = (int*) malloc(sizeof(int));
		 int *testData1 = (int*) malloc(sizeof(int));
		 int *testData2 = (int*) malloc(sizeof(int));
		 int *testData3 = (int*) malloc(sizeof(int));
		 int *testData4 = (int*) malloc(sizeof(int));
		 int *testData5 = (int*) malloc(sizeof(int));
		 bst_t* testTree = bst_construct(SearchPolicy);

		 printf("Path length with no entries: %d Expected: 0\n", bst_int_path_len(testTree));

		 bst_insert(testTree, ins[0], testData0);
		 printf("Path length with one entry: %d Expected: 0\n", bst_int_path_len(testTree));
		 
		 bst_insert(testTree, ins[1], testData1);
		 bst_insert(testTree, ins[2], testData2);
		 printf("Path length with three entries: %d Expected: 2\n", bst_int_path_len(testTree));
		 
		 bst_insert(testTree, ins[3], testData3);
		 printf("Path length with four entries: %d Expected: 4\n", bst_int_path_len(testTree));

		 bst_insert(testTree, ins[4], testData4);
		 printf("Path length with five entries: %d Expected: 6\n", bst_int_path_len(testTree));
		 
		 bst_insert(testTree, ins[5], testData5);
		 printf("Path length with six entries: %d Expected: 9\n", bst_int_path_len(testTree));

		 bst_destruct(testTree);
	 }
	 ...
	 if (UnitNumber == 8) {
		 ...
		 bst_insert(testTree, ins[0], testData0);
		 printf("Key comparisons for first insert: %d Expected: 0\n", bst_stats(testTree));
		 
		 bst_access(testTree, ins[0]);
		 printf("Key comparisons for first access: %d Expected: 1\n", bst_stats(testTree));

		 bst_insert(testTree, ins[1], testData1);
		 printf("Key comparisons for second insert: %d Expected: 2\n", bst_stats(testTree));
		 
		 bst_access(testTree, ins[1]); 
		 printf("Key comparisons for second access: %d Expected: 3\n", bst_stats(testTree));
		 
		 free(bst_remove(testTree, ins[1]));
		 printf("Key comparisons for first remove: %d Expected: 3\n", bst_stats(testTree));
		 
		 free(bst_remove(testTree, ins[0])); 
		 printf("Key comparisons for second remove: %d Expected: 1\n", bst_stats(testTree));
...
void equilibriumDriver(void)
{
    ...
    keys_added = keys_removed = 0;
	 // printf("Original size: %d Original bstsize: %d\n", size, bst_size(test_tree));
    start = clock();
    for (i = 0; i < Trials; i++) {
